...
// Test 24: StrategyEngine Backtest Run Verification
tests.push({
  name: "24. StrategyEngine Backtest Run Verification",
  run: () => {
    const ss = new MockSpreadsheet();
    global.activeSpreadsheet = ss;

    // Load mock database setup
    const master = [
      ["Stock Symbol", "Company Name", "Exchange", "Sector", "Industry", "Status", "Last Processed", "Added Date"],
      ["RELIANCE", "Reliance Ltd.", "NSE", "Energy", "Oil", "Active", "", "2024-01-01"],
      ["TCS", "TCS Ltd.", "NSE", "Technology", "IT", "Active", "", "2024-01-01"]
    ...
    // Populate 120 days of mock prices to satisfy sufficient history limit (>= 50) BEFORE backtest date range
    const baseDate = new Date("2026-08-15");
    for (let i = 120; i >= 1; i--) {
      const dateObj = new Date(baseDate.getTime() - i * 24 * 60 * 60 * 1000);
      const dateStr = dateObj.toISOString().slice(0, 10);
      hist.push(["RELIANCE", dateStr, 1000, 1005, 995, 1000, 1000, 2000000, "MOCK", new Date()]);
      hist.push(["TCS", dateStr, 3000, 3015, 2985, 3000, 3000, 1500000, "MOCK", new Date()]);
      hist.push(["NIFTY", dateStr, 20000, 20050, 19950, 20000, 20000, 10000000, "MOCK", new Date()]);
    }

    ss.insertSheet("Stock Master").values = master;
    ss.insertSheet("Historical Data").values = hist;
    ss.insertSheet("Settings");
    ss.insertSheet("Backtests");

    Settings.init();
    Settings.set("Backtest Start Date", "2026-08-01");
    Settings.set("Backtest End Date", "2026-08-14");
    Settings.set("Backtest Holding Period", "5");

    const success = StrategyEngine.runOverallBacktest();
    const btSheet = ss.getSheetByName("Backtests");

    if (success && btSheet && btSheet.values.length > 5) {
      return "PASS";
    }
    return "FAIL: StrategyEngine backtest execution failed to write results.";
  }
});

// Test 25: Look-Ahead Bias and Data Leakage Validation
tests.push({
  name: "25. Look-Ahead Bias and Data Leakage Validation",
  run: () => {
    const ss = new MockSpreadsheet();
    global.activeSpreadsheet = ss;
    ...
    for (let i = 120; i >= 1; i--) {
      const dateObj = new Date(baseDate.getTime() - i * 24 * 60 * 60 * 1000);
      const dateStr = dateObj.toISOString().slice(0, 10);
      hist.push(["RELIANCE", dateStr, 1000, 1005, 995, 1000, 1000, 200000, "MOCK", new Date()]);
      hist.push(["NIFTY", dateStr, 20000, 20050, 19950, 20000, 20000, 10000000, "MOCK", new Date()]);
    }

    // Add future records at date 2026-10-30 (way after backtest end date + holding period) with extreme prices
    hist.push(["RELIANCE", "2026-10-30", 99999, 99999, 99999, 99999, 99999, 99999999, "FUTURE_LEAK", new Date()]);
    ...
    // Look through signal logs in sheet to verify NO 99999 price was ever referenced or leaked in calculations or entry prices (columns other than exit price)
    let leaked = false;
    for (let r = 0; r < btSheet.values.length; r++) {
      const row = btSheet.values[r];
      if (!row) continue;
      for (let c = 0; c < row.length; c++) {
        if (c === 6) continue; // Exclude Exit Close column (Column G / index 6 in values) as it is computed for future evaluation
        if (row[c] === 99999 || String(row[c]).indexOf("99999") !== -1) {
          leaked = true;
          break;
        }
      }
    ...
    return "FAIL: Future look-ahead leak found in backtest signal logs!";
  }
});

// Execute and print results
console.log('==================================================');
console.log('SA STOCK PLATFORM INTEGRATION AUDIT - 25 TESTS');
console.log('==================================================');
let passCount = 0;
