...
class MainOrchestrator {
  ...
  static updateData() {
    ...
    try {
      ...
      // Automatically run sector rotation analysis to update dashboard
      try {
        SectorEngine.runSectorAnalysis();
      } catch (secErr) {
        Logger.error("MainOrchestrator.updateData - SectorEngine", 0, secErr);
      }
  ...
  /**
   * Orchestrates sector rotation calculation, history storage, and dashboard updates.
   */
  static runSectorAnalysis() {
    ...
    try {
      SectorEngine.runSectorAnalysis();
      var elapsed = new Date().getTime() - start;
      Logger.success("MainOrchestrator.runSectorAnalysis", elapsed);
    } catch (e) {
      var elapsed = new Date().getTime() - start;
      Logger.error("MainOrchestrator.runSectorAnalysis", elapsed, e);
      throw e;
    } finally {
      Logger.flush();
    }
  ...
  static generateReport() {
    ...
    try {
      ...
      try {
        ...
        Logger.error("MainOrchestrator.generateReport - SectorEngine", 0, secErr);
      }
